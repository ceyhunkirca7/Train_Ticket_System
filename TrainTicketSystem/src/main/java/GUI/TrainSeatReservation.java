...
public class TrainSeatReservation extends JFrame implements ActionListener {
    private static final String FILE_PATH = "seferYonetim.json";
    private JButton[][] seats;
    private JButton paymentButton, cancelButton, forwardButton, backwardButton;
    ...
    public TrainSeatReservation(Sefer selectedSefer, MusteriKayit musteriKayitx, int musteriIndex, String fromStop, String toStop) {
        ...
        paymentButton = new JButton("Ödemeye Devam Et");
        ...
        if (e.getSource() == paymentButton) {
            if (selectedSeat != null) {
                ...
                if (selectedSeatNumber != -1) {
                    User yolcu = musteriKayitx.getUsers().get(musteriIndex);
                    Bilet bilet = new Bilet(fromStop, toStop, selectedSefer, selectedSeatNumber);
                    if(yolcu.biletSatinAl(bilet)==-1){
                        JOptionPane.showMessageDialog(null, "Bakiye Yetersiz");
                    }else{
                        // Bilet başarıyla satın alındı mesajı gösterilebilir
                        yolcu.biletSatinAl(bilet);
                        //selectedSefer.biletEkle(bilet);
                        int i=0, selectedIndex1=-1;
                        ArrayList<Sefer> seferler=seferYonetimx.getSeferler();
                        for(Sefer x:seferler){
                            i++;
                        ...
                        if(selectedIndex1!=-1){
                            seferYonetimx.getSeferler().get(selectedIndex1-1).biletEkle(bilet);
                            saveDataToFile(seferYonetimx);
                        }                        
                        JOptionPane.showMessageDialog(null, "Bilet başarıyla satın alındı!");
                    }
                } else {
                    // Hiçbir koltuk seçilmediğinde uygun bir mesaj gösterilir
                    JOptionPane.showMessageDialog(null, "Hiçbir koltuk seçilmedi.");
                }
                //
                JOptionPane.showMessageDialog(this, "Tebrikler, " + selectedSeatNumber + " numaralı koltuk için bilet alındı!");
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Lütfen bir koltuk seçiniz.");
            }
        } else if (e.getSource() == cancelButton) {
            // İptal işlemleri burada yapılır
            JOptionPane.showMessageDialog(this, "İptal ediliyor...");
            dispose();
            ...
            if (selectedSeat != null) {
                selectedSeat.setBackground(null); // Eski seçili koltuğu eski haline döndür
            }
            if (selectedSeat == clickedSeat) {
                selectedSeat = null; // Tekrar aynı koltuğa tıklanırsa seçimi kaldır
            } else {
                selectedSeat = clickedSeat;
                selectedSeat.setBackground(Color.GREEN); // Yeni seçili koltuğu yeşil yap
            }
        }
    }
}